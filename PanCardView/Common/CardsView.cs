...
namespace PanCardView
{
    public class CardsView : Grid
    {
        ...
        protected virtual bool CheckIsProtectedView(IView view) => (view as View).Behaviors.Any(b => b is ProtectedControlBehavior);
        ...
        private void RemoveRangeViewsPool(IView[] views)
        {
            foreach (var view in views)
            {
                ...
                {
                    viewsCollection.Remove(view as View);
                }
            }
        }
        ...
        private void RemoveRedundantChildren(bool isProcessingNow)
        {
            ...
            InvokeOnMainThreadIfNeeded(() =>
            {
                lock (_childrenLocker)
                {
                    ...
                    var views = Children.Where(c => !CheckIsProtectedView(c) && !CheckIsProcessingView(c) && !_viewsInUseSet.Contains(c as View));
                    if (IsViewReusingEnabled)
                    {
        ...
        private void RemoveUnprocessingChildren()
        {
            InvokeOnMainThreadIfNeeded(() =>
            {
                lock (_childrenLocker)
                {
                    var views = Children.Where(c => !CheckIsProtectedView(c) && !CheckIsProcessingView(c));
                    RemoveChildren(views.ToArray());
                }
            });
        }

        private void RemoveChildren(IView[] views)
        {
        ...
        private bool CheckIsProcessingView(IView view)
